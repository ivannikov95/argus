      ...
      <main>
        ...
        <div className="workspace">
          ...
          {page === "dataset" && dataset && (
            <DatasetPage dataset={dataset} onUpload={() => inputRef.current?.click()} onOpenVariables={() => setPage("variables")} />
          )}
...
function DatasetPage({ dataset, onUpload, onOpenVariables }: { dataset: Dataset; onUpload: () => void; onOpenVariables: () => void }) {
  ...
  return (
    <section className="page">
      <div className="page-heading"><div><span className="eyebrow">01 · Подготовка данных</span><h1>Датасет</h1><p>Проверка структуры до любых статистических решений.</p></div><button className="button secondary" onClick={onUpload}>Импорт CSV / XLSX</button></div>
      ...
      <div className="panel">
        <div className="panel-head"><div><h2>{dataset.file_name}</h2><p>Первые 8 наблюдений · исходные значения не изменены</p></div><span className="status-ok">● Данные готовы</span></div>
        <div className="table-scroll"><table className="data-table"><thead><tr>{dataset.schema.map((v) => <th key={v.name}>{v.name}<small>{v.type}</small></th>)}</tr></thead><tbody>{dataset.rows.slice(0, 8).map((row, index) => <tr key={index}>{dataset.schema.map((v) => <td key={v.name}>{formatValue(row[v.name])}</td>)}</tr>)}</tbody></table></div>
        <div className="panel-footer"><span>Показано 8 из {dataset.row_count}</span><button className="text-button" onClick={onOpenVariables}>Проверить словарь переменных →</button></div>
      </div>
    </section>
  );
}
